...
const { currentProject } = await useProjectTimeStore()
...
const ui = {
  ...
  header: `flex flex-row bg-blue-800 font-bold text-white sm:px-2 px-2 sm:py-1 py-1 h-10`,
  body: "flex flex-col flex-nowrap gap-1 sm:p-0 p-0 sm:py-1 py-1 overflow-x-auto",
  footer: `flex flex-row bg-gray-200 sm:px-2 px-2 sm:py-1 py-1 items-center justify-between`,
}
...
<template>
  ...
  >
    <template #header>
      <IconLabel icon="fa7-solid:list-check" label="Projects" />
    </template>
    <template #footer>
      <UInput
        v-model="newProject"
    ...
    <UButton
      ...
      :color="project.color"
      class="w-full cursor-pointer h-10"
